...
(do :help
    ...
    (defn hydra-body-prelude [path-map hydra]
      (concat (when-let [docstring (hydra-docstring hydra)]
                [docstring])
              [(list "q" nil "quit" :exit 't :column "Nav")]
              (if-let [parent (get path-map (vec (butlast (:hydra/path hydra))))]
                [(list "<escape>" (:hydra/body-varsym parent)
                       (str ".." (clojure.core/name (last (:hydra/path parent)))) :exit 't :column "Nav")])))
    (defn without-qualified-keys[m]
      (->> (remove (comp qualified-keyword? key) m)
           (into {}))))

(defn parse-hydra
  [{:as parent :hydra/keys [path]}
   [hydra-name key x & xs]]
...
(defn expr-wrapper [f]
  (head-wrapper (fn [c] (update c :hydra/expr f))))
...
(do :compile
    ...
    (defn hydra-compile-head
      ...
      (if expr
        ...
        (list key body-varsym hint :exit 't :column "Sub")))
...
(comment
  (let [[input-file output-file] *command-line-args*]
    (spit output-file
          (pretty-str (hydra-compile (edn/read-string (slurp input-file)))))))
