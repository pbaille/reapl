...
local function action_path(p)
  local _30_ = u.tbl.path(p)
  if (nil ~= _30_) then
    return u.seq.interpose(_30_, "children")
  else
    return _30_
  end
end
...
local function do_action(p)
  ...
  do
    ...
    if (nil ~= _32_) then
      fn_path = u.seq.append(_32_, "fn")
    else
      fn_path = _32_
    end
  end
  local _34_ = u.tbl.get(reaper_actions, fn_path)
  if (nil ~= _34_) then
    local f = _34_
    return f()
  else
...
local function get_binding_tree()
  local function _37_(at, node)
    if ("table" == type(node)) then
      local f = node.fn
      if f then
        node["fn"] = str_join(u.seq["take-nth"](at, 2), ".")
      else
      end
      return node
    else
...
return {tree = reaper_actions, ["do-action"] = do_action, ["get-binding-tree"] = get_binding_tree}
